...
def jgr_rms_fields(TP, model=2, fdrive=0.022):
    ...
    PW = plotWindow(ncols=len(azms), nrows=len(fields), colorbar='log')

    # Title and labels.
    title = notex('RMS Magnetic Field Components: ') + tex(model) + notex(', ') + tex(fdrive) + notex('Driving')
    ulab = notex('\\frac{nJ}{m^3}')
    clabs = [ 'm \\! = \\! ' + str(azm) for azm in azms ]
    rlabs = [ notex('Poloidal'), notex('Toroidal'), notex('Compressional')  ]
    ...
    for col, azm in enumerate(azms):
        ...
        if path is None:
            PW[:, col].setParams( text=notex('Not Found') )
            continue
...
def jgr_energy(TP, models=(2,), fdrives=(0.022,)):
    ...
    PW = plotWindow(ncols=len(azms), nrows=len(models), colorbar=None)

    title = notex('Integrated Poloidal (Blue) and Toroidal (Red) Energy')

    clabs = [ 'm \\! = \\! ' + str(azm) for azm in azms ]

    rlabs = []
    for model, fdrive in zip(models, fdrives):
        rlab = tex(model) + notex('\n') + tex(fdrive)
        rlabs.append(rlab)

    PW.setParams(collabels=clabs, rowlabels=rlabs, title=title)

    for col, azm in enumerate(azms):
        for row, (model, fdrive) in enumerate( zip(models, fdrives) ):
...
def jgr_layers(TP, model=2, fdrive=0.022):
    ...
    PW = plotWindow(ncols=len(azms), nrows=2, colorbar='log')

    title = notex('Poloidal and Toroidal Energy Density: ') + tex(model) + notex(', ') + tex(fdrive) + notex('Driving')
    clabs = [ 'm \\! = \\! ' + str(azm) for azm in azms ]
    rlabs = (
        notex('Poloidal\nEnergy\nDensity'),
        notex('Toroidal\nEnergy\nDensity'),
    )
    PW.setParams(collabels=clabs, unitlabel=tex('mW/m^2'), rowlabels=rlabs, title=title, zmax=0.1, ylabelpad=0)

    for col, azm in enumerate(azms):
        path = TP.getPath(azm=azm, fdrive=fdrive, model=model)

        PW[:, col].setParams( **TP.getCoords(path, 't', 'L0') )
        ...
        [ PW[i, col].setContour(uofL) for i, uofL in enumerate(uofLpt) ]
...
def jgr_ground(TP, model=2, fdrive=0.022):
    ...
    PW = plotWindow(ncols=len(azms), nrows=len(fields), colorbar='sym')

    title = notex('Magnetic Ground Signatures: ') + tex(model) + notex(', ') + tex(fdrive) + notex('Driving')
    clabs = [ 'm \\! = \\! ' + str(azm) for azm in azms ]
    rlabs = [ tex( x[:2] ) for x in fields ]
    PW.setParams(collabels=clabs, unitlabel=notex('nT'), rowlabels=rlabs, title=title, zmax=100)

    for col, azm in enumerate(azms):
        path = TP.getPath(azm=azm, fdrive=fdrive, model=model)

        PW[:, col].setParams( **TP.getCoords(path, 't', 'lat0') )
...
def jgr_grounds(TP, models, fdrives):
    ...
    PW = plotWindow(ncols=len(azms), nrows=len(models)*len(fields), colorbar='sym')
    ...
    rlabs = []
    for model in models:
        rlabs += [ tex(model) + tex( x[:2] ) for x in fields]

    PW.setParams(collabels=clabs, unitlabel=notex('nT'), rowlabels=rlabs, title=title, zmax=100, ylabelpad=0)
    ...
    for col, azm in enumerate(azms):
        ...
        for i, (model, fdrive) in enumerate( zip(models, fdrives) ):
            ...
            PW[2*i:2*i+2, col].setParams( **TP.getCoords(path, 't', 'lat0') )

            Bs = [ TP.getArray(path, x)[:, 0, :] for x in fields ]
            Bmaxes = [ np.max( np.abs( x[5:-5, 5:-5] ) ) for x in Bs ]

            [ PW[2*i+j, col].setContour(x) for j, x in enumerate(Bs) ]

            [ PW[2*i+j, col].setParams( bottext=notex('Max: ') + format(x, '.0f') + notex('nT') ) for j, x in enumerate(Bmaxes) if x > np.sqrt(10.) ]
...
def plotSymh(TP, showline=False):
  ...
  filename = '/home/charles/Desktop/symh_20130601.txt'
  ...
  ticks = (-3, -2, -1, 0, 1)
  ticklabels = ('$-3$', '', '$-1$', '', '$+1$')
  ylabel = notex('Log Amplitude (nT \\, min)')
